  ...
  switch (Number(UserChoice)) {
    ...
    case eChoice.eDOUBLE_ATK: {// 연속공격
      ...
      if (DoubleAtkPer < player.double_atk) { // 확률에 따라 연속 공격
        ...
        let MonsterHitDamage2 = await monster.hit(await player.attack());
    ...
    case eChoice.eRUNAWAY: {// 도망치기
      if (await player.runAwayFunc()) {// runAwayFunc 함수 호출해서 도망 확률 계산
        logs.push(chalk.cyan(`플레이어는 성공적으로 몬스터에게서 도망쳤다!!!`));
        return eStage.eRUNAWAY_END;
      }
      else logs.push(chalk.red(`플레이어는 도망치기를 실패하였다!!!`));
      break;
    }
    case eChoice.eEXIT: {
  ...
  switch (Number(MonsterChoice)) {
    ...
    case eChoice.eDOUBLE_ATK: {// 연속공격
      ...
      if (DoubleAtkPer < monster.double_atk) { // 확률에 따라 연속 공격
        ...
        let PlayerHitDamag2 = await player.hit(await monster.attack());
    ...
    default: {// 예외처리 에러코드 호출용
      logsError.push(chalk.red(`switch: default - Number(MonsterChoice) : ${Number(MonsterChoice)}`));
      break;
    }
  }
}
...
const battle = async (stage, player, monster) => {
  ...
  while (player.CurrentHp > 0 && monster.CurrentHp > 0) {
    ...
    // 플레이어 로직 함수
    battleEndState = await PlayerManager(logs, logsError, UserChoice, player, monster);
